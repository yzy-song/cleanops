      ...
      <div className="mx-auto max-w-2xl px-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Find the perfect name for your cleaning company</p>
          </div>
        </div>
          ...
          <CardContent className="pt-6 space-y-4">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {styles.map((s) => (
                  <button key={s} onClick={() => setStyle(s)} className={`rounded-full px-4 py-1.5 text-sm border transition-colors ${style === s ? "bg-primary text-primary-foreground border-primary" : "bg-card border-border hover:border-primary/50"}`}>
                    {s}
                  </button>
                ))}
              </div>
