...
```bash
# 1. 创建目录结构
sudo mkdir -p /var/www/cleanops-api/{releases,backups,logs}
sudo chown -R $USER:$USER /var/www/cleanops-api

# 2. 创建 Git 裸仓库
git init --bare /var/www/cleanops-api/repo.git

# 3. 拷贝 post-receive hook
cp deploy/post-receive /var/www/cleanops-api/repo.git/hooks/post-receive
chmod +x /var/www/cleanops-api/repo.git/hooks/post-receive

# 4. 拷贝部署脚本
cp deploy/deploy.sh /var/www/cleanops-api/deploy.sh
cp deploy/rollback.sh /var/www/cleanops-api/rollback.sh
cp deploy/ecosystem.config.js /var/www/cleanops-api/ecosystem.config.js
chmod +x /var/www/cleanops-api/deploy.sh /var/www/cleanops-api/rollback.sh

# 5. 创建 .env 文件
cat > /var/www/cleanops-api/.env << 'EOF'
...
git remote add production ssh://user@your-server:/var/www/cleanops-api/repo.git
...
# 回滚到指定版本 (时间戳即目录名)
/var/www/cleanops-api/rollback.sh 20240830120000
```
...
# 查看日志
pm2 logs cleanops-api

# 重启
pm2 restart cleanops-api
...
| 项目 | 旧方案 | 新方案 |
|------|--------|--------|
| 项目名 | cloudloom-server | cleanops-api |
